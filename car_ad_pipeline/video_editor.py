...
def compile_ad(raw_video: str, scene_cues: list, tts_audios: list, bg_music: str, subtitle_ass: str, output_dir: str, final_path: str):
    ...
    if sfx_track_path and os.path.exists(sfx_track_path):
        ...
        cmd_final = [
            ...
            "-i", bg_music,
            "-i", sfx_track_path,
            "-filter_complex", f"[0:a]volume=1.0[v_a];[1:a]volume=0.25[m_a];[2:a]volume=0.45[s_a];[v_a][m_a][s_a]amix=inputs=3:duration=first:normalize=0[a];[0:v]{vf_filter}[v]",
            "-map", "[v]",
            "-map", "[a]",
            "-c:v", "libx264",
            "-crf", "21",
        ...
        cmd_final = [
            ...
            "-i", concatenated_raw,
            "-i", bg_music,
            "-filter_complex", f"[0:a]volume=1.0[v_a];[1:a]volume=0.25[m_a];[v_a][m_a]amix=inputs=2:duration=first:normalize=0[a];[0:v]{vf_filter}[v]",
            "-map", "[v]",
            "-map", "[a]",
            "-c:v", "libx264",
            "-crf", "21",
